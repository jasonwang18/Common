...
public final class ServiceManager {
     ...
     * @param startId
     */
    public void onStart(Context context, Intent proxyIntent, int startId) {
        ...
        try {
            ...
            if (!mServiceMap.containsKey(serviceInfo.name)) {
                // service还不存在, 先创建
                proxyCreateService(context, serviceInfo);
            }

            Service service = mServiceMap.get(serviceInfo.name);
            service.onStartCommand(targetIntent, START_FLAG_REDELIVERY, startId);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * @return
     */
    public int stopService(Intent targetIntent, Context context) {
        ServiceInfo serviceInfo = selectPluginService(targetIntent);
        if (serviceInfo == null) {
            Log.w(TAG, "can not found service: " + targetIntent.getComponent());
            return 0;
        ...
        if (mServiceMap.isEmpty()) {
            ...
            context.stopService(new Intent().setComponent(new ComponentName(context.getPackageName(), ProxyService.class.getName())));
        }
        return 1;
    }
     ...
     * @throws Exception
     */
    private void proxyCreateService(Context context, ServiceInfo serviceInfo) throws Exception {
     ...
     * @throws Exception 解析出错或者反射调用出错, 均会抛出异常
     */
    public void preLoadServices(Context context, File apkFile) throws Exception {

        PackageInfo packageInfo = context.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), PackageManager.GET_SERVICES);

        // 解析出intent对应的Service组件
        for (ServiceInfo service : packageInfo.services) {

            mServiceInfoMap.put(new ComponentName(service.packageName, service.name), service);
        }
    }
}
