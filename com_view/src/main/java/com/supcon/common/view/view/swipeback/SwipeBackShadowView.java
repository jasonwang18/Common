...
class SwipeBackShadowView extends FrameLayout {
     ...
     * 阴影资源 id
     */
    private int mShadowResId = R.drawable.bg_sbl_shadow;
    /**
     * 阴影区域的透明度是否根据滑动的距离渐变
     */
    ...
    void bindPreActivity() {
        ...
        if (mPreActivity == null) {
            Activity preActivity = App.getAppContext().getPenultimateActivity(mActivity);
            if (preActivity != null) {
    ...
    void unBindPreActivity(boolean isNeedAddImageView) {
        ...
        if (mPreDecorView != null && mPreContentView != null) {
            ...
            if (!(mPreContentView instanceof SwipeBackLayout)) {
                ...
                int height = mPreDecorView.getHeight() - Util.getNavigationBarHeight(activity);
                if (!Util.isPortrait(activity)) {
                    width = mPreDecorView.getWidth() - Util.getNavigationBarHeight(activity);
    ...
    private Bitmap getBitmap(View view) {
        ...
        Bitmap bitmap = Bitmap.createBitmap(view.getDrawingCache(), 0, 0,
                Util.getRealScreenWidth(mActivity),
                Util.getRealScreenHeight(mActivity) - Util.getNavigationBarHeight(mActivity));
    ...
    private boolean containsProblemView(ViewGroup viewGroup) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (App.getAppContext().isProblemView(childView)) {
                return true;
    ...
    private void onPanelSlide(Activity currentActivity, float slideOffset) {
        try {
            Activity preActivity = App.getAppContext().getPenultimateActivity(currentActivity);
            if (preActivity != null) {
